...
{
    public class MyRc5W32 : MyRc5//WordType - for a and b
    {
        ...
        public MyRc5W32(string password, int keyLength, byte numberOfRounds)
        {
            //Type type = typeof (WordType);
            _mW = sizeof(uint);//8;//(byte) (Marshal.SizeOf(type)*8);//8;//64 bits
        ...
        private void EncryptStream(Stream inputStream, Stream outputStream)
        {
            ...
            a0 = (uint) myRandom.random();//generate random a and b
            b0 = (uint) myRandom.random();
            int w2 = (_mW << 1);
            ...
            {//main cycle
                ...
                if (readBytesCount == 0)
                    break;
                a = BitConverter.ToUInt32(buffer, 0);//get a
                b = BitConverter.ToUInt32(buffer, _mW);//get b
            ...
            {
                a = BitConverter.ToUInt32(buffer, i * w2);
                b = BitConverter.ToUInt32(buffer, i * w2 + _mW);
                EncryptBlock(ref a0, ref b0, ref a, ref b, s);
        ...
        private bool DecryptStream(Stream inputStream, Stream outputStream)
        {
            ...
            do
            {
                ...
                b = BitConverter.ToUInt32(buffer, _mW);
        ...
        private void EncryptBlock(ref uint a0, ref uint b0, ref uint a, ref uint b, uint[] s)
        {
            a ^= a0;
            b ^= b0;
            a += s[0];
        ...
        private void DecryptBlock(ref uint a0, ref uint b0, ref uint a, ref uint b, uint[] s)
        {
            var prevA = a;
            var prevB = b;
        ...
        private uint[] GenerateSubKeys(byte[] key)
        {
            ...
            var c = (uint)(keyLength / _mW);//K length in words
            var l = new uint[c];
            Buffer.BlockCopy(key, 0, l, 0, keyLength);
            uint subkeysCount = (uint)(2 * _numberOfRounds + 2);//counts of subkeys
            var s = new uint[subkeysCount];//subleys
            s[0] = _mPw;
            for (i = 1; i < subkeysCount; i++)
                s[i] = s[i - 1] + _mQw;
            i = j = 0;
            uint a = 0, b = 0;
            for (uint k = 0; k < 3 * Math.Max(subkeysCount, c); k++)
            {
        ...
        uint RotateLeft(uint value, int offset, bool reverse = false)
        {
        ...
        private uint _mPw = Pw32;
        private uint _mQw = Qw32;
